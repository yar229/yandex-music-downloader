...
def main():
    ...
    if args.add_lyrics:
        _logger.warning('Аргумент --add-lyrics устарел и будет удален в будущем. Используйте --lyrics-format')
    ...
    if args.url is not None:
        ...
        if match := ARTIST_RE.search(path):
            ...
            _logger.info('Параметер url указан в неверном формате')
            return 1
    ...
    if args.artist_id is not None:
        ...
        def filter_album(album: Album) -> bool:
            ...
            if album.id is None or not album.available:
                _logger.warning(f'Альбом "{title}" не доступен для скачивания')
                
            elif args.only_music and album.meta_type != "music":
                _logger.warning(f'Альбом "{title}" пропущен т.к. не является музыкальным')
            elif args.stick_to_artist and album.artists[0].id != int(args.artist_id):
                _logger.warning(f'Альбом "{title}" пропущен из-за флага --stick-to-artist')
            else:
                return True
            return False
    ...
    for track in result_tracks:
        ...
        if not track.available:
            _logger.warning(f'{progress_status}Трек {get_artists(track)} - {track.title} не доступен для скачивания')
            continue
        ...
        format_info += "]"
        _logger.info(f'{progress_status}{format_info} Загружается {downloadable.path}')
        core.download_track(
